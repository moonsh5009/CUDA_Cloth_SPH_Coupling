...
__global__ void getNumSPHParticleContactElements_kernel(
	SPHParticleParam sphParam,
	ObjParam objParam, BVHParam objBvh, RTriParam objRTri,
	uint* ceSize)
{
	...
	if (threadIdx.x == 0u) {
		...
		if (ino > sphParam._numParticles)
			s_endNum = ino - sphParam._numParticles;
		else s_endNum = 0u;
		s_bvttNum = 0u;
	}

	uint objileaf = objBvh._size - objParam._numFaces;
	uint path = 0;
	uint level = 0;
	...
	uint inos[3];
	REAL3 pis[3], pj;
	uchar typeis[3], typej;
	REAL thicknessi, thicknessj;

	bool isLoop = true, isIntersected;

	if (id >= sphParam._numParticles)
		isLoop = false;
	else {
		getParticleElements_Detection_device(sphParam, id, pj, typej, thicknessj);
		setAABB(aabb, pj, thicknessj * COL_CLEARANCE_RATIO);
	}
	...
	do {
		...
		if (isLoop) {
			icomp = getBVHIndex(path, level);
			getBVHAABB(comp._aabb, objBvh, icomp);
			isIntersected = intersect(aabb, comp._aabb);
			if (isIntersected && icomp < objileaf) {
				level++;
				path <<= 1u;
			}
			else {
				if (isIntersected) {
					comp._face = objBvh._faces[icomp - objileaf];
					ino = atomicAdd(&s_bvttNum, 1u);
					s_bvtts[0][ino] = comp._face;
					s_bvtts[1][ino] = id;
				}
		...
		if (s_endNum >= blockDim.x || s_bvttNum >= BVTT_SHARED_SIZE - blockDim.x) {
			for (ino = threadIdx.x; ino < s_bvttNum; ino += blockDim.x) {
				lRTri = objRTri._info[s_bvtts[0][ino]];
				getMeshElements_Detection_device(objParam, s_bvtts[0][ino], inos, pis, typeis, thicknessi);
				getParticleElements_Detection_device(sphParam, s_bvtts[1][ino], pj, typej, thicknessj);
				numParticleContactElements_device(
					lRTri,
					inos, pis, typeis,
					s_bvtts[1][ino], pj, typej,
					(thicknessi + thicknessj) * 0.5, num);
			}
			__syncthreads();
			if (threadIdx.x == 0u)
				s_bvttNum = 0u;
...
__global__ void getSPHParticleContactElements_kernel(
	SPHParticleParam sphParam,
	ObjParam objParam, BVHParam objBvh, RTriParam objRTri,
	ContactElemParam ceParam)
{
	...
	do {
		...
		if (s_endNum >= blockDim.x || s_bvttNum >= BVTT_SHARED_SIZE - blockDim.x) {
			for (ino = threadIdx.x; ino < s_bvttNum; ino += blockDim.x) {
				...
				getMeshElements_Detection_device(objParam, s_bvtts[0][ino], inos, pis, typeis, thicknessi);
				getParticleElements_Detection_device(sphParam, s_bvtts[1][ino], pj, typej, thicknessj);
				getParticleContactElements_device(
					lRTri,
					inos, pis, typeis,
					s_bvtts[1][ino], pj, typej,
					(thicknessi + thicknessj) * 0.5, ces, ceSize);
...
__global__ void getSPHParticleCCDtime_kernel(
	SPHParticleParam sphParam,
	ObjParam objParam, BVHParam objBvh, RTriParam objRTri,
	const REAL dt, REAL* minTime)
{
	...
	if (id >= sphParam._numParticles)
		...
		getParticleElements_CCD_device(sphParam, id, pj, qj, typej, thicknessj, dt);
		setAABB(aabb, pj, COL_CCD_THICKNESS);
		addAABB(aabb, qj, thicknessj);
	}
	...
	do {
		...
		if (s_endNum >= blockDim.x || s_bvttNum >= BVTT_SHARED_SIZE - blockDim.x) {
			for (ino = threadIdx.x; ino < s_bvttNum; ino += blockDim.x) {
				lRTri = objRTri._info[s_bvtts[0][ino]];
				getMeshElements_CCD_device(objParam, s_bvtts[0][ino], inos, pis, qis, typeis, thicknessi, dt);
				getParticleElements_CCD_device(sphParam, s_bvtts[1][ino], pj, qj, typej, thicknessj, dt);
				getCCDTime_device(true, pis[0], pis[1], pis[2], pj, qis[0], qis[1], qis[2], qj, (thicknessi + thicknessj) * 0.5, &t);
			}
			__syncthreads();
			if (threadIdx.x == 0u)
				s_bvttNum = 0u;
