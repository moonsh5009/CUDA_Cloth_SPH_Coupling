	...
	bool isProximity, REAL friction, REAL thickness, REAL dt)
{
	...
	REAL iP2 = w2 * w2;
	REAL iP3 = w3 * w3;
	REAL iPt = (iP0 + iP1 + iP2 + iP3) * (invM0 + invM1 + invM2 + invM3);*/
	...
	if (iPt > 0.0) {
		...
		//REAL imp_r = 0.0;// min(dt * stiffness * d, 0.1 * d / dt - relVN);
		//REAL imp_c = (max(min(thickness, dist), thickness * 0.25) - dist) / dt - relVN;
		...
		//}
		//else {
		//	imp = (max(min(thickness, dist), thickness * 0.25) - dist) / dt - relVN;
...
__global__ void compDetectedRigidImpactZone_CE_kernel(
	...
	ObjParam clothParam, ObjParam obsParam, SPHParticleParam sphParam,
	...
	bool isProximity, const REAL dt)
{
	...
	ContactElem ce = ceParam._elems[id];
	//if (isProximity && ce._type[3] == TYPE_SPH_PARTICLE)
	//	return;
	...
	for (uint i = 0u; i < 4u; i++) {
 		if (ce._type[i] == TYPE_SPH_PARTICLE) {
			uint phase = sphParam._phases[ce._i[i]];
			REAL s = sphParam._ss[ce._i[i]];
			invMs[i] = sphParam._masses[phase];
			invMs[i] = 1.0 / (invMs[i] * s + FLT_EPSILON);
			...
			//friction += friction;
			friction = 0.0;
			impulses[i] = sphParam._impulses + ce._i[i] * 3u;
			colWs[i] = sphParam._colWs + ce._i[i];
		}
		else {
			if (ce._type[i] == TYPE_MESH_CLOTH) {
				param = &clothParam;
...
//	}
//}
__global__ void applySPHParticleCollision_kernel(
	SPHParticleParam sphParam, REAL dt, bool* isApplied)
{
	uint id = threadIdx.x + blockDim.x * blockIdx.x;
	if (id >= sphParam._numParticles)
		return;

	REAL colW = sphParam._colWs[id];
	if (colW > 0.0) {
		uint ino = id * 3u;
		REAL3 impulse;
		impulse.x = sphParam._impulses[ino + 0u];
		impulse.y = sphParam._impulses[ino + 1u];
		impulse.z = sphParam._impulses[ino + 2u];
		impulse *= 1.0 / colW;

		*isApplied = true;

		REAL3 v;
		v.x = sphParam._vs[ino + 0u];
		v.y = sphParam._vs[ino + 1u];
		v.z = sphParam._vs[ino + 2u];
		v += impulse;

		sphParam._vs[ino + 0u] = v.x;
		sphParam._vs[ino + 1u] = v.y;
		sphParam._vs[ino + 2u] = v.z;
	}
}
__global__ void applyClothCollision_kernel(
	ObjParam clothParam, REAL dt, bool* isApplied)
	...
	ObjParam param, uint* inbNs, uint* nbNs, REAL* prevImpulses, REAL* impulses)
{
	...
	ino = id * 3u;
	impulse.x = prevImpulses[ino + 0u];
	impulse.y = prevImpulses[ino + 1u];
	impulse.z = prevImpulses[ino + 2u];
	if (!param._isFixeds[id]) {
		...
		for (i = istart; i < iend; i++) {
			ino = nbNs[i]; ino *= 3u;
			nbImp.x = prevImpulses[ino + 0u];
			nbImp.y = prevImpulses[ino + 1u];
			nbImp.z = prevImpulses[ino + 2u];
			lnbImp = Length(nbImp);
			lnbImp *= limp;
		...
		limp = Length(impulse);
		nbImps.x = nbImps.y = nbImps.z = 0.0;
		ws = 0.0;
		for (i = istart; i < iend; i++) {
			ino = nbNs[i]; ino *= 3u;
			nbImp.x = prevImpulses[ino + 0u];
		...
		impulse += nbImps * LAPLACIAN_MU;
#endif
	}
	ino = id * 3u;
	impulses[ino + 0u] = impulse.x;
	impulses[ino + 1u] = impulse.y;
	impulses[ino + 2u] = impulse.z;
}
	...
	RIZoneParam riz, const REAL dt)
{
	...
	for (i = istart; i < iend; i++) {
		...
		if (ino.y == TYPE_MESH_CLOTH) {
			...
			uint phase = sphParam._phases[ino.x];
			REAL s = sphParam._ss[ino.x];
			mass = sphParam._masses[phase];
			mass *= s;
			getVector(sphParam._xs, ino.x, p);
			getVector(sphParam._vs, ino.x, v);
		}
	...
	if (lomg) {
		...
		for (i = istart; i < iend; i++) {
			...
			if (ino.y == TYPE_MESH_CLOTH) {
				...
				setVector(sphParam._vs, ino.x, v);
			}
		}
	}
}
